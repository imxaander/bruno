...
| **Vault**                   | A tiered family of powerful special cards. The catalog cards form the offer pool; the deck holds 9 vault tokens (5 silver / 3 gold / 1 diamond), each offering up to 5 random same-tier effects sampled from cards with a registered resolver (see `game/vault-mechanism.md`). |
